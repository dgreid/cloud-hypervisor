...
/// remain valid for as long as Self is alive.
///
/// The `iovecs` array is held in a `Vec` so that callers can hand its address
/// to the kernel before the operation is stashed in an in-flight map: moving
/// the `GuestMemoryTarget` (and the `AsyncIoOperation` that owns it) does not
/// relocate the heap allocation, so the pointer remains valid until the
/// request completes. `Vec` is preferred over `Box<[_]>` here so the storage
...
mod tests {
    ...
    fn iovecs_survive_move() {
        ...
        // is moved into an in-flight map. Capture the pointer before the move
        // and confirm it still matches afterwards.
        ...
        let iovec_ptr_before = target.iovecs().as_ptr();
        let iov_base_before = target.iovecs()[0].iov_base;

        let moved = Box::new(target);

        assert_eq!(moved.iovecs().as_ptr(), iovec_ptr_before);
        assert_eq!(moved.iovecs()[0].iov_base, iov_base_before);
